...
from include.custom_wherobots_provider.hooks.hooks_rest_api_custom import CUSTOMWherobotsRestAPIHook
...
class CUSTOMWherobotsRunOperator(BaseOperator):
    """
    ...
    def poll_and_display_logs(
        self, hook: CUSTOMWherobotsRestAPIHook, run: Run, start: int
    ) -> int:
        """
    ...
    def _wait_run_poll_logs(self, hook: CUSTOMWherobotsRestAPIHook, run: Run):
        logs_cursor: int = 0
    ...
    def _tail_post_run_logs(
        self, hook: CUSTOMWherobotsRestAPIHook, run: Run, last_cursor
    ) -> None:
    ...
    def _wait_run_simple(self, hook: CUSTOMWherobotsRestAPIHook, run: Run) -> Run:
    ...
    def execute(self, context) -> Any:
        ...
        """
        with CUSTOMWherobotsRestAPIHook(self.wherobots_conn_id) as rest_api_hook:
